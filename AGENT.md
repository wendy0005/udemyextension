...
- `content-script.js` – Self-invoking module that exposes `window.udemyTranscriptHelper` with helper functions:
  ...
  - `exportSelectionToCsv(selection?)` → gathers transcripts for the chosen lectures, generates a CSV, and triggers a download.
...
5. Validate on a Udemy course page via DevTools console:
   ...
   ```
6. Upon load the content script automatically (after a ~7 s grace period to let Udemy finish rendering):
   - Watches the DOM (mutation observer + polling) until the curriculum list appears.
   - Renders a floating “Transcript Helper” checklist that lists only the playable video lectures (identified via the `#icon-video` SVG). The user can select/deselect any combination, then press “Export selected to CSV” to drive automation.
   - During export the agent clicks each selected video in order, opens the transcript toggle, copies all transcript lines, closes the transcript sidebar, and repeats until finished. A CSV named `udemy-transcripts-<timestamp>.csv` downloads when complete.
...
7. To trigger the panel or exports manually:
   ...
   await window.udemyTranscriptHelper.exportSelectionToCsv(); // re-use last selection from the UI
   ```
...
- When automating navigation through lessons, reuse `showSelectionPanel()` / `findCurriculumItems()` so the agent can honor whatever combination of video lectures the user selects before export.
