...
- `content-script.js` – Self-invoking module that exposes `window.udemyTranscriptHelper` with helper functions:
  ...
  - `exportSelectionToText(selection?)` → gathers transcripts for the chosen lectures, formats plain text, and triggers a `.txt` download (reports progress + skipped lectures).
...
6. Activation:
   ...
   - During export the agent clicks each selected video in order, opens the transcript toggle, copies all transcript lines, closes the transcript sidebar, and repeats until finished. The downloaded text file is named after the course title plus the number of selected videos (e.g., `Course Name (16 videos).txt`) and groups transcript lines per lecture. A progress bar on the helper panel updates per lecture so the user can see how many videos remain.
